...
import { isSafeHttpUrl, traceabilityLabel } from "@/lib/official-sources";
...
export function streetViewUrl(point: PuntoEncuentro): string {
  const params = new URLSearchParams({
    api: "1",
    map_action: "pano",
    viewpoint: `${point.latitud},${point.longitud}`,
  });
...
export default function InteractiveMap() {
  ...
  useEffect(() => {
    ...
    const loadExclusionZone = async () => {
      ...
      const { data } = await supabase
        ...
        .select("id, nivel_alerta, radio_km, descripcion, fuente, documento, fecha_fuente, trazabilidad")
    ...
    void import("leaflet").then((L) => {
      ...
      if (exclusionZone && Number.isFinite(exclusionZone.radio_km) && exclusionZone.radio_km > 0) {
        L.circle([-39.42, -71.93], {
          ...
          weight: 2,
          dashArray: "6 6",
        }).addTo(layer).bindTooltip(tooltipText(`${traceabilityLabel(exclusionZone.trazabilidad)}: ${exclusionZone.radio_km} km`));
      }
      L.marker([-39.42, -71.93], { icon: volcanoIcon }).addTo(layer).bindTooltip(tooltipText("Volcán Villarrica — monitoreo técnico"));
  ...
  return (
    <section className="space-y-5" aria-labelledby="map-title">
      ...
      {exclusionZone ? <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 rounded-lg border border-border/70 bg-card/50 p-3 text-sm"><strong>Zona de exclusión referencial</strong><Badge variant="outline" className="border-primary/30 text-primary">{traceabilityLabel(exclusionZone.trazabilidad)}</Badge><span className="font-mono text-primary">{exclusionZone.radio_km} km</span><span className="text-muted-foreground">{exclusionZone.descripcion}</span><span className="text-xs text-muted-foreground">Fuente: {exclusionZone.fuente || "No declarada"}</span>{isSafeHttpUrl(exclusionZone.fuente_url) ? <a className="inline-flex items-center gap-1 text-xs text-primary hover:underline" href={exclusionZone.fuente_url} target="_blank" rel="noreferrer">Abrir fuente <ExternalLink className="size-3" aria-hidden="true" /></a> : null}{exclusionZone.documento ? <span className="text-xs text-muted-foreground">Documento: {exclusionZone.documento}</span> : null}{exclusionZone.fecha_fuente ? <span className="text-xs text-muted-foreground">Fecha: {formatLocalDateTime(exclusionZone.fecha_fuente)}</span> : null}</div> : null}
      ...
      <div>
        ...
        {points.length === 0 ? <div className="rounded-xl border border-dashed border-border bg-card/40 p-10 text-center"><MapPin className="mx-auto size-8 text-muted-foreground" aria-hidden="true" /><p className="mt-3 font-display font-semibold">Sin puntos registrados</p><p className="mt-1 text-sm text-muted-foreground">Registra el primero desde el panel de operación.</p></div> : <div className="grid gap-3 md:grid-cols-2">{points.map((point) => <Card key={point.id} className={`border-border/80 bg-card/60 transition-colors ${selectedPoint === point.id ? "border-primary/60 ring-1 ring-primary/30" : ""}`}><CardContent className="p-4"><button type="button" onClick={() => setSelectedPoint(point.id)} className="w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{point.nombre}</p><p className="mt-1 text-sm text-muted-foreground">{point.direccion}</p></div><div className="flex flex-wrap justify-end gap-1"><Badge variant="outline" className={point.ocupado ? "border-red-300/40 bg-red-400/10 text-red-200" : "border-emerald-300/40 bg-emerald-400/10 text-emerald-200"}>{point.ocupado ? "Lleno" : "Disponible"}</Badge><Badge variant="outline" className="border-primary/30 text-primary">{traceabilityLabel(point.trazabilidad)}</Badge></div></div><div className="mt-4 grid grid-cols-3 gap-2 border-t border-border/60 pt-3 text-xs"><span><strong className="block font-mono text-foreground">{point.capacidad}</strong><span className="text-muted-foreground">capacidad</span></span><span><strong className="block font-mono text-foreground">{point.tiempo_aprox_pie} min</strong><span className="text-muted-foreground">a pie</span></span><span><strong className="block font-mono text-foreground">{point.seguridad_nivel}/5</strong><span className="text-muted-foreground">seguridad</span></span></div><div className="mt-3 border-t border-border/60 pt-3 text-xs text-muted-foreground"><p>Fuente: {point.fuente || "No declarada"}</p>{isSafeHttpUrl(point.fuente_url) ? <a className="mt-1 inline-flex items-center gap-1 text-primary hover:underline" href={point.fuente_url} target="_blank" rel="noreferrer">Abrir fuente <ExternalLink className="size-3" aria-hidden="true" /></a> : null}{point.documento ? <p className="mt-1">Documento: {point.documento}</p> : null}{point.fecha_fuente ? <p className="mt-1">Fecha: {formatLocalDateTime(point.fecha_fuente)}</p> : null}</div></button><div className="mt-3 flex flex-wrap gap-2"><Button asChild size="sm" variant="outline" className="min-h-10 flex-1" disabled={point.ocupado || !isValidCoordinate(point.latitud, point.longitud)}><a href={point.ocupado || !isValidCoordinate(point.latitud, point.longitud) ? undefined : navigationUrl(point)} target="_blank" rel="noreferrer" aria-disabled={point.ocupado || !isValidCoordinate(point.latitud, point.longitud) || undefined} tabIndex={point.ocupado || !isValidCoordinate(point.latitud, point.longitud) ? -1 : undefined}><Navigation aria-hidden="true" /> Navegar</a></Button><a href={isValidCoordinate(point.latitud, point.longitud) ? `https://www.google.com/maps/search/?api=1&query=${point.latitud},${point.longitud}` : undefined} target="_blank" rel="noreferrer" aria-disabled={!isValidCoordinate(point.latitud, point.longitud) || undefined} tabIndex={!isValidCoordinate(point.latitud, point.longitud) ? -1 : undefined} className="inline-flex min-h-10 items-center gap-1.5 rounded-md px-3 text-xs text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Ver mapa <ExternalLink className="size-3" aria-hidden="true" /></a><a href={isValidCoordinate(point.latitud, point.longitud) ? streetViewUrl(point) : undefined} target="_blank" rel="noreferrer" aria-disabled={!isValidCoordinate(point.latitud, point.longitud) || undefined} tabIndex={!isValidCoordinate(point.latitud, point.longitud) ? -1 : undefined} aria-label={`Ver RA (Street View) de ${point.nombre}`} title="Street View" className="inline-flex min-h-10 items-center gap-1.5 rounded-md px-3 text-xs text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Ver RA <ExternalLink className="size-3" aria-hidden="true" /></a></div></CardContent></Card>)}</div>}
      </div>
    </section>
  );
}

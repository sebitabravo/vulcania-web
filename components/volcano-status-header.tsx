...
import { AlertTriangle, Activity, TrendingUp, Shield, Clock } from "lucide-react"
...
export default function VolcanoStatusHeader() {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "alertas_volcan" }, () => {
        cargarAlerta()
      })
      .subscribe()
  ...
  return (
    <div className={`border-b border-gray-800 bg-gradient-to-r ${config.bgGradient}`}>
      <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <div className="flex items-center space-x-3 mb-1">
                <h2 className="text-xl font-semibold text-white">Volcán Villarrica</h2>
                <Badge className={`${config.color} ${config.textColor} font-medium`}>{config.label}</Badge>
              </div>
              <p className="text-gray-300 text-sm mb-1">{alerta.descripcion}</p>
              <div className="flex items-center space-x-2 text-xs text-gray-400">
                <Clock className="h-3 w-3" />
                <span>Actualizado {calcularTiempoTranscurrido(alerta.ultima_actualizacion)}</span>
              </div>
            </div>
          </div>
        </div>
